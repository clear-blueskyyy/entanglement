...
export function buildEntangleSystemPrompt(): string {
  ...
  return [
    {
      ...
      strategy: "balanced",
      system:
        "这一轮优先求妙感成立。先选最合适的妙感类型和枢纽节点，再写 1-3 条路径，优先 2 条。",
    },
    {
      timeoutMs: ATTEMPT_TIMEOUTS_MS[1],
      strategy: "anti_generic",
      system:
        "这一轮重点修复泛化。删掉万能桥接词、鸡汤总结、流水账式硬中介堆砌；宁可少，不要凑。",
    },
    {
      timeoutMs: ATTEMPT_TIMEOUTS_MS[2],
      strategy: "single_path",
      system:
        "这一轮只求 1 条高置信路径。请逐跳自检 `connectionToNext`、`detail`、`summary` 是否真的具体，不要空话。",
    },
  ];
}
