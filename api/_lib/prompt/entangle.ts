...
function buildBaseSystemPrompt(
  ...
  return [
    ...
    '4. `connectionFromStart`：你是在解释"为什么起点会把故事推向第一个中间节点"。必须写清什么资源、权力、资金、数据、货物、人力或信息，通过什么机制，从起点传到第一个中间节点。坏写法示例："由此建立隐性通道"、"议题转向"、"两者相关"。',
    '5. `connectionToNext`（每个节点各写一条）：你是在解释"为什么这个节点会把故事推向下一跳"。必须写清什么资源、权力、资金、数据、货物、人力或信息，通过什么机制，从本节点传到下一节点。坏写法示例："由此建立隐性通道"、"议题转向"、"两者相关"。',
    '6. `detail`：你是在解释"这个节点为什么值得出现"。要写现实约束、制度边界、商业结构、媒介作用或物质条件。坏写法示例："关键一环"、"重要组成"、"放大器"。',
    '7. `summary`：40-100 字，总结整条路径到底妙在哪。必须点明妙感类型，说明枢纽节点如何让起点走到终点；不要复述节点列表，不要写"看似 A 其实 B"这种万能句型。',
    "",
    ...
    '{"paths":[{"title":"4-10字标题","hook":"一句钩子，说明这条路线从哪里拐过去，并回扣起点与终点","surprise":"一句话指出最妙的意外点，并回扣起点与终点","surpriseIndex":8,"connectionFromStart":"起点为何会把故事推向第一个中间节点，要写具体机制","nodes":[{"term":"节点1","connectionToNext":"节点1为何会把故事推向节点2，要写具体机制","detail":"这个节点为何关键，要写现实信息"},{"term":"节点2","connectionToNext":"节点2为何会把故事推向节点3，要写具体机制","detail":"这个节点为何关键，要写现实信息"},{"term":"节点3","connectionToNext":"节点3为何会把故事推向终点，要写具体机制","detail":"这个节点为何关键，要写现实信息"}],"summary":"40-100字，总结这条路径真正妙在哪里，并明确点回起点与终点"}]}',
