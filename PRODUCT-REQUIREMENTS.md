...
**星线数据**：共 40 条连线，每条为 `[fromIndex, toIndex]` 的二元组，渲染为 `<line>` 元素。
...
**结果态降级**（`.app.has-result .card-hero`）：
...
容器 `.pair-input-shell`：`display: grid; grid-template-columns: 1fr auto 1fr; gap: 16px; align-items: end;`
...
| 文案 | `换一条更妙的路径` |
| 颜色 | `var(--cool-white)` |
| 背景 | `rgba(20,24,36,0.6)` |
| 边框 | `1px solid var(--border-mid)` |
| Hover 背景 | `rgba(30,34,48,0.8)` |
...
按 `Date.now() / 1200 % 3` 轮换，每 1.2 秒切换：

| 索引 | 文案 |
|------|------|
| 0 | `正在沿着星图找第一条能站得住的暗线……` |
| 1 | `先绕开最直白的答案，试着把两个世界缝到同一张图上……` |
| 2 | `正在把中间那几次关键转折一点点拎出来……` |
...
| `onResetExperience` | `() => void` | 重置回调（"换一对试试"） |
...
**派生状态**：
- `isComplete = totalMiddleCount > 0 && revealedCount >= totalMiddleCount`
...
**右侧控制按钮组** `.path-controls.compact`：

| 按钮 | 类名 | 文案（未完成/已完成） | 禁用条件 |
|------|------|----------------------|----------|
| 揭示下一跳 | `.button` | `揭示下一跳（{revealedCount}/{totalMiddleCount}）` / `整条路径已经点亮` | `revealedCount >= totalMiddleCount` |
...
| 文案 | 第 1 条边：`"先让两端进入同一张图"`；其余：`activePath.nodes[index-1].connectionToNext` |
...
- 说明：`首页只做一件事：让你把两个遥远的世界丢进来，看看它们会不会连上。`（13px, `var(--text-secondary)`, 行高 1.7）

**步骤 02**：
- 序号：`02`
- 标题：`一跳一跳揭示`
- 说明：`中间的节点不会一次摊平，而是由你主动点击，把最关键的那几次拐弯点亮。`

**步骤 03**：
- 序号：`03`
- 标题：`最后才亮出全貌`
- 说明：`当所有节点都被点亮，页面会给出概览与下一步引导，让这条暗线真正收束。`
...
| 仪式 01 标题 | 抛出一对词 |
| 仪式 01 说明 | 首页只做一件事：让你把两个遥远的世界丢进来，看看它们会不会连上。 |
| 仪式 02 标题 | 一跳一跳揭示 |
| 仪式 02 说明 | 中间的节点不会一次摊平，而是由你主动点击，把最关键的那几次拐弯点亮。 |
| 仪式 03 标题 | 最后才亮出全貌 |
| 仪式 03 说明 | 当所有节点都被点亮，页面会给出概览与下一步引导，让这条暗线真正收束。 |
...
| E3 | Hero 收缩 | `result` 出现 | 600ms | var(--ease) | scale(0.92) + translateY(-16px) + opacity 0.35 |
...
2. **动效完整性**：E1–E13 所有动效必须实现，时长与缓动参数不得偏差超过 50ms
